...
#define MAX_CHECK_DATAFLASH_AREA_RETRY_COUNT    3
    ...
    #endif /* if SFD_CONTROL_BLOCK_SIZE == 1024 */

#else  /* if defined( BSP_MCU_RX65N ) || ( BSP_MCU_RX72N ) */
    #error "Simple Filesystem on Dataflash (SFD) does not support your MCU"
#endif /* if defined( BSP_MCU_RX65N ) || ( BSP_MCU_RX72N ) */
...
R_BSP_ATTRIB_SECTION_CHANGE_END R_BSP_ATTRIB_SECTION_CHANGE( C,
                                                             SFD_MIRROR_SECTION_NAME,
                                                             1 )
static const SFD_CONTROL_BLOCK sfd_control_block_data_mirror =
{
    SFD_CONTROL_BLOCK_INITIAL_DATA
}
...
SFD_HANDLE R_SFD_SaveObject( uint8_t * label,
...
SFD_HANDLE R_SFD_FindObject( uint8_t * label,
...
sfd_err_t R_SFD_Scan( uint8_t ** label,
                      uint32_t * label_length,
                      uint8_t ** data,
                      uint32_t * data_length )
{
