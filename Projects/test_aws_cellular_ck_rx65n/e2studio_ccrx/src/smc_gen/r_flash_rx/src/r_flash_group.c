...
***********************************************************************************************************************/
    ...
    #define FLASH_RETURN_IF_PCFG_NULL                \
    if( ( pcfg == NULL ) || ( pcfg == FIT_NO_PTR ) ) \
    {                                                \
        return FLASH_ERR_NULL_PTR;                   \
    }

    #define FLASH_RETURN_IF_ROM_LT_32K \
...
/* See FLASH_SAS_xxx #defines */
    #define FLASH_RETURN_IF_BAD_SAS                 \
    if( ( *pSwapInfo == 1 ) || ( *pSwapInfo > 4 ) ) \
    {                                               \
    ...
    if( ( flash_ready_isr_handler == FIT_NO_FUNC ) || ( flash_ready_isr_handler == NULL ) ) \
    {                                                                                       \
        return FLASH_ERR_FAILURE;                                                           \
    }
#else
...
static flash_err_t flash_interrupt_config( bool state,
                                           void * pcfg );
static flash_err_t get_erase_flash_type( flash_block_address_t block_start_address,
                                         uint32_t num_blocks,
                                         flash_type_t * type );
static bool is_cf_addr( uint32_t addr );
#if ( FLASH_CFG_PARAM_CHECKING_ENABLE && FLASH_CFG_CODE_FLASH_ENABLE )
    static bool is_cf_overflow( uint32_t addr,
                                uint32_t num_bytes );
    #if FLASH_ERASE_CF_ASCENDING_BLOCK_NUMS
        static void get_cf_addr_info( uint32_t addr,
                                      flash_addr_info_t * info );
    #endif
#endif
static flash_err_t set_erase_params( flash_block_address_t block_start_address,
                                     uint32_t num_blocks,
                                     flash_type_t flash_type );
#ifndef FLASH_NO_BLANK_CHECK
    static flash_err_t set_blankcheck_params( uint32_t address,
                                              uint32_t num_bytes,
                                              flash_type_t flash_type );
#endif
static flash_err_t get_bc_pgm_flash_type( uint32_t address,
                                          uint32_t num_bytes,
                                          flash_type_t * flash_type );
static flash_err_t set_write_params( uint32_t address,
                                     uint32_t num_bytes,
                                     flash_type_t flash_type );
#if ( FLASH_HAS_NON_CACHED_RANGES && FLASH_CFG_CODE_FLASH_ENABLE )
    static flash_err_t set_non_cached_regs( flash_non_cached_t * p_cfg,
                                            flash_non_cached_regs_t * p_regs );
#endif
...
***********************************************************************************************************************/
flash_err_t flash_interrupt_config( bool state,
                                    void * pcfg )
{
...
***********************************************************************************************************************/
                                            ...
                                            flash_non_cached_regs_t * p_regs )
    {
        ...
        #endif /* if FLASH_CFG_PARAM_CHECKING_ENABLE */
...
***********************************************************************************************************************/
FLASH_PE_MODE_SECTION
flash_err_t get_erase_flash_type( flash_block_address_t block_start_address,
                                  uint32_t num_blocks,
                                  flash_type_t * flash_type )
{
    *flash_type = FLASH_TYPE_INVALID;
    ...
    if( is_cf_addr( block_start_address ) )
    {
            ...
            *flash_type = FLASH_TYPE_CODE_FLASH;
                ...
                #endif /* if FLASH_ERASE_CF_ASCENDING_ADDRESSES */
                #if FLASH_ERASE_CF_ASCENDING_BLOCK_NUMS
...
***********************************************************************************************************************/
    ...
    #endif /* if FLASH_ERASE_CF_ASCENDING_BLOCK_NUMS */
...
***********************************************************************************************************************/
FLASH_PE_MODE_SECTION
flash_err_t set_erase_params( flash_block_address_t block_start_address,
                              uint32_t num_blocks,
                              flash_type_t flash_type )
{
...
***********************************************************************************************************************/
    FLASH_PE_MODE_SECTION
    flash_err_t set_blankcheck_params( uint32_t address,
                                       uint32_t num_bytes,
                                       flash_type_t flash_type )
    {
        FLASH_RETURN_IF_BGO_AND_NO_CALLBACK;
        ...
        else /* CODE FLASH */
        {
                ...
                #endif /* if ( FLASH_HAS_CF_BLANK_CHECK == 0 ) */
            #endif /* if ( FLASH_CFG_CODE_FLASH_ENABLE == 1 ) */
        }
...
***********************************************************************************************************************/
FLASH_PE_MODE_SECTION
flash_err_t get_bc_pgm_flash_type( uint32_t address,
                                   uint32_t num_bytes,
                                   flash_type_t * flash_type )
{
    *flash_type = FLASH_TYPE_INVALID;
...
***********************************************************************************************************************/
FLASH_PE_MODE_SECTION
flash_err_t set_write_params( uint32_t address,
                              uint32_t num_bytes,
                              flash_type_t flash_type )
{
    FLASH_RETURN_IF_BGO_AND_NO_CALLBACK;
