 ...
 ******************************************************************************/
    ...
    static sci_err_t sci_receive_async_data( sci_hdl_t const hdl,
                                             uint8_t * p_dst,
                                             uint16_t const length );
#endif /* if ( SCI_CFG_ASYNC_INCLUDED || SCI_CFG_IRDA_INCLUDED ) */
...
static void sci_receive( sci_hdl_t const hdl );
    ...
    static sci_err_t sci_send_sync_data_dma_dtc( sci_hdl_t const hdl,
                                                 uint8_t * p_src,
                                                 uint8_t * p_dst,
                                                 uint16_t const length );
#endif
    ...
    static sci_err_t sci_send_sync_data_fifo_dma_dtc( sci_hdl_t const hdl,
                                                      uint8_t * p_src,
                                                      uint8_t * p_dst,
                                                      uint16_t const length,
                                                      bool save_rx_data );
 ...
 ******************************************************************************/
                                          ...
                                          uint16_t const length )
    {
        ...
        byteq_err_t byteq_err = BYTEQ_ERR_QUEUE_FULL;
            ...
            #endif
        #else  /* if ( SCI_CFG_USE_CIRCULAR_BUFFER == 1 ) */
            if( true != hdl->tx_idle )
            {
                return SCI_ERR_XCVR_BUSY;
            }
        #endif /* if ( SCI_CFG_USE_CIRCULAR_BUFFER == 1 ) */
        #if SCI_CFG_FIFO_INCLUDED
            if( true == hdl->fifo_ctrl )
            {
                    ...
                    #endif /* if ( TX_DTC_DMACA_ENABLE & 0x01 || TX_DTC_DMACA_ENABLE & 0x02 ) */
                #else  /* if ( SCI_CFG_USE_CIRCULAR_BUFFER == 1 ) */
                    /* TX FIFO use check */
                    if( 0x00 < hdl->rom->regs->FDR.BIT.T )
                    {
                    ...
                    #endif
                #endif /* if ( TX_DTC_DMACA_ENABLE != 0 ) */
                                         ...
                                         bool save_rx_data )
    {
        ...
        if( true == hdl->tx_idle )
        {
            ...
            #endif /* SCI_CFG_FIFO_INCLUDED */
            {
                ...
                #endif /* if ( TX_DTC_DMACA_ENABLE & 0x01 ) */
                {
                    #if ( TX_DTC_DMACA_ENABLE & 0x02 )
                        if( SCI_DMACA_ENABLE == hdl->rom->dtc_dmaca_tx_enable )
                        {
                    ...
                    #endif /* if ( TX_DTC_DMACA_ENABLE & 0x02 ) */
                    {
                        hdl->tx_cnt--;
                        SCI_TDR( *hdl->u_tx_data.buf ); /* start transmit */
                    }
 ...
 ******************************************************************************/
                                             ...
                                             uint16_t const length )
    {
        ...
        #endif /* if ( RX_DTC_DMACA_ENABLE & 0x01 ) */
        {
            ...
            #endif /* if ( RX_DTC_DMACA_ENABLE & 0x02 ) */
            {
                uint16_t cnt;
        ...
        if( true == hdl->tx_idle )
        {
            ...
            hdl->tx_dummy = true;
                ...
                if( true == hdl->fifo_ctrl )
                {
                    ...
                    #endif /* if ( SCI_DTC_DMACA_DISABLE != RX_DTC_DMACA_ENABLE ) */
                    {
                         ...
                         void * p_args )
{
    ...
    switch( cmd )
    {
        ...
        case ( SCI_CMD_SET_TXI_PRIORITY ):
            ...
            #endif /* if defined( BSP_MCU_RX64M ) || defined( BSP_MCU_RX71M ) || defined( BSP_MCU_RX65N ) || defined( BSP_MCU_RX66T ) || defined( BSP_MCU_RX72T ) || defined( BSP_MCU_RX72M ) || defined( BSP_MCU_RX72N ) || defined( BSP_MCU_RX66N ) || defined( BSP_MCU_RX671 ) || defined( BSP_MCU_RX660 ) */
